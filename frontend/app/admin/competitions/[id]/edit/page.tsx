...
import { updateCompetitionDetails, CreateCompetitionRequest } from '@/lib/api/admin';
...
function EditCompetitionContent() {
  ...
  // Pre-fill form when competition data loads
  useEffect(() => {
    if (competition) {
      setTitle(competition.title);
      setDescription(competition.description);
      setDomain(competition.domain);
      setEntryFee(String(competition.entry_fee));
      setMaxEntries(String(competition.max_entries));

      // Format dates for datetime-local input
      const openDateTime = new Date(competition.open_date);
      const deadlineDateTime = new Date(competition.deadline);
      setOpenDate(openDateTime.toISOString().slice(0, 16));
      setDeadline(deadlineDateTime.toISOString().slice(0, 16));

      setJudgingSLA(String(competition.judging_sla_days));
      setPlatformFee(String(competition.platform_fee_percentage));

      // Parse rubric
      if (competition.rubric) {
        const criteria = Object.entries(competition.rubric).map(([name, data]: [string, any]) => ({
          name,
          weight: data.weight || 0,
          description: data.description || '',
        }));
        setRubricCriteria(criteria.length > 0 ? criteria : [{ name: '', weight: 0, description: '' }]);
      }

      // Parse prize structure
      if (competition.prize_structure) {
        const prizeEntries = Object.entries(competition.prize_structure).map(([place, percentage]) => ({
          place,
          percentage: Number(percentage),
        }));
        setPrizes(prizeEntries.length > 0 ? prizeEntries : [{ place: 'first', percentage: 0 }]);
      }

      // Load existing image if available
      if (competition.image_url) {
        setExistingImageUrl(competition.image_url);
      }
    }
  }, [competition]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: CreateCompetitionRequest) => {
      return await updateCompetitionDetails(competitionId, data);
    },
    onSuccess: () => {
  ...
  const confirmUpdate = async () => {
    ...
    try {
      // Update competition details first
      console.log('Updating competition:', competitionId);
      console.log('Update data:', data);
      await updateCompetitionDetails(competitionId, data);
      console.log('Competition details updated successfully');

      // Handle image removal if flagged
      if (removeExistingImage) {
        console.log('Removing existing image for competition:', competitionId);
        try {
          await api.delete(`/competitions/${competitionId}/image`);
          ...
          alert(
            'Competition updated, but failed to remove old image. ' +
            'You can try removing it again by editing the competition.\n\n' +
            `Error: ${deleteError.response?.data?.detail || deleteError.message}`
          );
        }
      }
      ...
      if (imageFile) {
        console.log('Uploading new image for competition:', competitionId);
        console.log('Image file:', imageFile);
        console.log('API Base URL:', process.env.NEXT_PUBLIC_API_URL);
        console.log('Full upload URL:', `${process.env.NEXT_PUBLIC_API_URL}/api/v1/competitions/${competitionId}/upload-image`);
        ...
        try {
          const imageResponse = await api.post(
            `/competitions/${competitionId}/upload-image`,
            imageFormData,
            {
          ...
          console.error('Image error status:', imageError.response?.status);

          // Don't fail the whole update, just log the error and show alert
          alert(
            'Competition updated successfully, but image upload failed. ' +
            'You can try uploading an image again by editing the competition.\n\n' +
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <Alert variant="error">Competition not found</Alert>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900">Edit Competition</h1>
        ...
        {updateMutation.isError && (
          <Alert variant="error" className="mb-6">
            {(updateMutation.error as any)?.response?.data?.detail || 'Failed to update competition'}
          </Alert>
        )}
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute top-3 right-3 flex gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          setImageSrc(imagePreview || existingImageUrl || '');
                      ...
                      <button
                        ...
                        className="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors duration-200"
                      >
                        Remove
                      </button>
                    </div>
              ...
              <div className="p-6">
                        ...
                        <div className="flex items-center justify-between mb-3 text-sm">
                          <div>
                            ...
                            <span className="font-bold text-slate-900">
                              ${competition?.prize_pool ? parseFloat(String(competition.prize_pool)).toFixed(2) : '0.00'}
                            </span>
                          </div>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${competition?.current_entries && maxEntries ? (competition.current_entries / parseInt(maxEntries)) * 100 : 0}%`
                              }}
                            />
                          </div>
                          <span className="text-xs font-semibold text-slate-600 whitespace-nowrap">
                            {competition?.current_entries || 0}/{maxEntries || '0'}
                          </span>
                        </div>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 mb-6 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-900 capitalize">{competition?.status || 'Draft'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-600">Current Entries:</span>
                    <span className="font-semibold text-slate-900">{competition?.current_entries || 0}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-600">Current Prize Pool:</span>
        ...
        {showImageCropper && imageSrc && (
          <ImageCropperModal
            ...
            onCropComplete={(croppedFile) => {
              ...
              setRemoveExistingImage(false); // Clear removal flag if uploading new image
            }}
            onClose={() => {
              setShowImageCropper(false);
